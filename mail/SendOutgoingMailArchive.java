 ...
 */
public class SendOutgoingMailArchive implements WrRuleClassBody {
    ...
    public Object run(Map parameters) throws ScriptException {
        ...
        try {
            ...
            workSession.notifyUser(creator, null, null, 1, "The mail " + name + " has been sent", "entity/" + mail.getId());

            // let's signal the mail to archive it
            mail.signal(instanceId, taskId);
            // then move it under the correct folder
            String containerName = (String) mail.getContainerEntity().getPropertyValue("name");
            if (containerName != null && containerName.equalsIgnoreCase("draft")) {
                manager.moveMail(mail, "Outgoing");
            }
            result.put("entityId", mail.getId());
        } catch (Exception ex) {
            logger.error("Gestione errore invio Mail...",ex);
            String errorMsg = ex.getMessage();
            if (errorMsg != null && errorMsg.indexOf(":") > 0) {
                errorMsg = errorMsg.substring(errorMsg.indexOf(":") + 1);
            }
            if (errorMsg != null && errorMsg.indexOf("Invalid email") > -1) {
                errorMsg = "Indirizzo Email non valido, controllare i dati e riprovare.";
            } else if (errorMsg != null && errorMsg.indexOf("Sending failed, attachments") > -1 || errorMsg.indexOf("Sending failed, file") > -1) {
